...
class Args():
    def __init__(self):
        ...
        # Salvo i parametri su file JSON (stringa)
        with open(saveloc + 'params.json', 'w') as f:
            f.write(str(self.getParamsDict()))

    def getParamsDict(self):
        ret = {key: value for key, value in self.__dict__.items() if not key.startswith('__') and not callable(value)}
        print('\nHYPERPARAMETERS = ', ret)
        return ret
